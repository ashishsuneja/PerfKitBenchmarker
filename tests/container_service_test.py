...
class ContainerServiceTest(pkb_common_test_case.PkbCommonTestCase):
    ...
    def setUp(self):
        ...
        self.kubernetes_cluster = container_service_mock.CreateTestKubernetesCluster()
    ...
    def test_GetNumReplicasSamples_found(self):
        ...
        self.MockIssueCommand(
            {f"get {resource_name} -n {namespace} -o=jsonpath=": [("456, 123", "", 0)]}
        )
    ...
    def test_GetNumReplicasSamples_not_found(self):
        ...
        samples = kubernetes_commands.GetNumReplicasSamples(resource_name, namespace)
    ...
    def test_GetKubectlEvents_Success(self, unused_mock):
        ...
        self.assertEqual(
            ...
            kubernetes_events.KubernetesEvent(
                kubernetes_events.KubernetesEventResource(kind="ConfigMap", name=None),
                message="gke-49fe-vm became leader",
                reason="LeaderElection",
                type="Normal",
    ...
    def test_GetPodNamesForResource_success(self):
        ...
        names = kubernetes_commands._GetPodNamesForResource(resource_name, namespace)
    ...
    def test_GetCPURequestSamples_success(self):
        ...
        with mock.patch.object(
            ...
            samples = kubernetes_commands.GetCPURequestSamples(resource_name, namespace)
...
class KubernetesClusterSyncWrappersTest(pkb_common_test_case.PkbCommonTestCase):
    ...
    def setUp(self):
        ...
        self.cluster.CreateNodePool = lambda cfg, node_version=None: kubernetes_cluster.KubernetesCluster.CreateNodePool(
            self.cluster, cfg, node_version
        )
        self.cluster.DeleteNodePool = (
            lambda name: kubernetes_cluster.KubernetesCluster.DeleteNodePool(
                self.cluster, name
            )
        )
        self.cluster.UpgradeNodePool = (
            lambda name, version: kubernetes_cluster.KubernetesCluster.UpgradeNodePool(
                self.cluster, name, version
            )
        )
        self.cluster.UpdateCluster = (
            lambda: kubernetes_cluster.KubernetesCluster.UpdateCluster(self.cluster)
        )
