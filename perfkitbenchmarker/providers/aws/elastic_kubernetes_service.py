...
class EksKarpenterCluster(BaseEksCluster):
    ...
    def _Create(self):
        ...
        create_json: dict[str, Any] = {
            ...
            "managedNodeGroups": [self._RenderNodeGroupJson(bootstrapping_nodepool)],
        }
        self._EksCtlCreate(create_json)
    ...
    def _PostIngressNetworkingFixups(
        ...
        host = (
            parse.urlparse(address).hostname if address.startswith("http") else address
        )
    ...
    def _PostCreate(self):
        ...
        self.alias_version = (
            "v" + full_version.strip().strip('"').split(f"{self.cluster_version}-v")[1]
        )
    ...
    def _CreateKarpenterNodePool(self, nodepool: container.BaseNodePoolConfig):
        ...
        if nodepool.min_nodes == nodepool.max_nodes:
            ...
            cpu_limit = max(1000, math.ceil(nodepool.max_nodes * vcpu_per_node * 1.05))
    ...
    def _CleanupKarpenter(self):
        ...
        kubectl.RunRetryableKubectlCommand(
            ...
            timeout=300,
            suppress_failure=lambda stdout, stderr, retcode: (
                "no resources found" in stderr.lower() or "not found" in stderr.lower()
            ),
        )
